...
# --- 1) extend the Post interface ---
old_iface = '''  img: string;
  imgAlt: string;
  readMin: number;
}'''
new_iface = '''  img: string;
  imgAlt: string;
  readMin: number;
...
n0 = c.count(old_iface)
c = c.replace(old_iface, new_iface)

# --- 2) attach body / faq / internal links to the low-odor article ---
anchor = '''    img: U('1567789884554-0b844b597180'),
    imgAlt: 'Quality testing of low-emission tape material',
    readMin: 8,
  },'''

body = r'''    img: U('1567789884554-0b844b597180'),
    imgAlt: 'Quality testing of low-emission tape material',
    readMin: 8,
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Thickness</td><td>0.05 – 0.20 mm (customisable)</td><td>Thinner means less emitting mass — do not over-specify</td></tr>
...
n1 = c.count(anchor)
c = c.replace(anchor, body)

with open(path, 'w', encoding='utf-8') as f:
    f.write(c)
print(f'interface: {n0}, article: {n1}')
